...
const createKeyboard = () => {
  ...
  const keyboardHTML = KEYBOARD_LETTERS.reduce((acc, curr) => {
    return (
      acc +
      `<button id="${curr}" class="button-primary keyboard-button">
          ${curr}
        </button>`
    );
  }, "");
...
export const startGame = () => {
  ...
  const wordsToGuess = WORDS[randomIndex];
  sessionStorage.setItem("word", wordsToGuess);
